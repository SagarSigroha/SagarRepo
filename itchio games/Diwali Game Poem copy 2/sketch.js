...
function preload() {
  // intentionally empty — large assets are loaded in setup() so loading screen displays
}
...
function checkIfLoaded() {
  ...
  if (loadedCount >= totalToLoad) {
    loading = false;
    try { if (typeof enableAudio === 'function') enableAudio(); } catch(e){}
  }
}
...
function setup() {
  ...
  bgMusic = loadSound('props/bg.mp3', () => { bgMusic.setLoop(true); bgMusic.setVolume(0.3); checkIfLoaded(); }, () => { checkIfLoaded(); });
  crackers = loadSound('props/crackers.mp3', () => { crackers.setLoop(true); crackers.setVolume(0.2); checkIfLoaded(); }, () => { checkIfLoaded(); });
  footstep = loadSound('props/footstep.mp3', () => { footstep.setVolume(0.8); checkIfLoaded(); }, () => { checkIfLoaded(); });

  for (let i = 1; i <= 14; i++) {
    ((idx) => {
      loadSound('props/s' + idx + '.mp3', (s) => { s.setVolume(0.5); familySounds.push(s); checkIfLoaded(); }, () => { checkIfLoaded(); });
    })(i);
  }
}
...
function draw() {
  ...
  let pct = floor((loadedCount / totalToLoad) * 100);
  textSize(14);
  text(pct + "%", width / 2, height / 2 + 36);
  return;
}
